...
#Computes the "degree" k elements of the nice sphere constructor
def degree_sequence(g_vector, dimension, level):
    facets = sphere_constructor(g_vector, dimension)
    if facets  == None or level > facets[1] or level < 0:
        print "Error"
        return
    list_1 = []
    for i in range(2, len(facets)):
        add = True
        for j in range(facets[1]- level):
            if not  facets[i][j]==2*j+1:
                add = False
        if facets[i][facets[1]-level] == 2*(facets[1]-level)+1:
            add = False

        if add:
            list_1.append[facets[i]]
